...
const StatusAnalise: React.FC<{
  ...
  const isConcluido = analise.status === 'Concluído' || analise.status === 'Concluido';
  return (
...
export default function OportunidadesTabela() {
  ...
  const licitacoesExibidas = useMemo(() => {
    ...
    if (filtroTipo) {
      licitacoesProcessadas = licitacoesProcessadas.filter((l) => getClassificacao(l.objeto_compra) === filtroTipo);
    }
    if (ordemValor) {
                ...
                <div className="space-y-3 max-h-64 overflow-auto">
                  {ragAnswers.map((m, idx) => (
                    <div key={idx} className="text-sm">
                      ...
                      <div className="whitespace-pre-wrap mt-1 border rounded p-2 bg-gray-50">{m.a}</div>
                    </div>
                  ))}
                </div>
              )}
      ...
      <div className="rounded-lg border bg-white shadow-sm">
        <div className="max-h-[70vh] overflow-y-auto overflow-x-auto">
          {licitacoesExibidas.length > 0 && (
            <DataTable<Licitacao>
              data={licitacoesExibidas}
              rowKey={(l) => l.id}
              selectable
              selectedKeys={new Set(Array.from(selectedIds))}
              onToggleRow={(key) => handleSelect(Number(key))}
